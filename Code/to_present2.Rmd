...
```{r graph of villages with trees receiving bat visits by village type and trees observed per village, evaluate=FALSE, echo=FALSE}
# # Second version is better. Commenting this one out.
...
```{r}
# total trees visits and trees that received visits by buffet village type and village type for 6 tree types visited 
bat_tree_tot_f %>% 
  left_join(all_data %>% dplyr::select(Tree_number, Trees_observed)) %>% 
  distinct() %>% 
  ...
  mutate(yn_visit_rc = case_when(yn_visit == "yes" ~ 1,
                                 TRUE ~ 0)) %>% 
  group_by(Trees_observed, Tree_type) %>% 
  summarise(n_tree = n(), 
            n_tree_visited = sum(yn_visit_rc)) %>%
  mutate(total_n_tree = (cumsum(n_tree)), 
         total_n_tree_visit = (cumsum(n_tree_visited)), 
...
```
Still waiting on updated list of winter fruit names - currently using matching fruit names from trees observed for camera traps.
...
fruit_names_winter <- c("buroi", "guava", "star.fruit", "sofeda", "banana", "indian.olive",                          "betel", "jackfruit", "papaya", "date.palm")
...
```{r fruit tree presence in bari graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=10}
...
=======
* Betel and banana are the most common trees present in a bari at about 25%. 
* Far control villages report a higher proportion with betel and banana than other village types. 
* Other winter fruiting trees are rare in baris (< 5%).

```{r winter fruit tree presence in bari graph, echo=FALSE, fig.align=, fig.height=5, fig.width=10}
# create a new object where average consumption across village types is ranked
rank_tp_grouped_mean <- tp_grouped %>%
  subset(fruit %in% fruit_names_winter) %>% 
>>>>>>> Stashed changes
  filter(measure == "mean") %>%
  group_by(fruit) %>%
...
<<<<<<< Updated upstream
```{r fruit tree abundance in bari graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=10}
# create a new object where average abundance across village types is ranked
rank_tp_number_grouped_mean <- tp_number_grouped %>%
  filter(measure == "mean") %>%
  group_by(fruit) %>%
  mutate(average = mean(tp_fruit)) %>%
  arrange(desc(average)) %>%
...
# pull out the ranked fruit names
rank_tp_number_grouped_names <- rank_tp_number_grouped_mean %>%
  distinct(fruit) %>%
  pull(fruit)

# plot fruit tree abundance with x-axis ranked by the average
rank_tp_number_grouped_mean %>%
  filter(measure == "mean") %>%
  ggplot(aes(x = factor(rank), y = tp_fruit, color = case_control_group)) +
  geom_jitter(height = 0, width = 0.2, alpha = 0.8) +
  scale_x_discrete(labels = rank_tp_number_grouped_names) +
  labs(x = "Fruit type",
       y = "Number of trees in bari",
       title = "Number of trees present in a bari grouped by village type") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.y = element_text(vjust = 1)) +
  scale_color_manual(
...
```{r fruit tree presence in bari by buffet type graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=10}
...
=======
```{r winter fruit tree presence in bari by buffet type graph, echo=FALSE, fig.align=, fig.height=5, fig.width=10}
# create a new object where average consumption across village types is ranked
...
rank_tp_presence_grouped2_mean %>%
  ...
  labs(x = "Fruit type",
       y = "Proportion of baris with tree type",
       title = "Proportion of respondants reporting winter fruting tree presence in bari grouped by village type") +
  theme_bw() +
...
```{r fruit tree abundance in bari by buffet type graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=10}
# create a new object where average abundance across village types is ranked
rank_tp_number_grouped2_mean <- tp_number_grouped2 %>%
  filter(measure == "mean") %>%
  group_by(fruit) %>%
  mutate(average = mean(tp_fruit2)) %>%
  arrange(desc(average)) %>%
...
# pull out the ranked fruit names
rank_tp_number_grouped2_names <- rank_tp_number_grouped2_mean %>%
  distinct(fruit) %>%
  pull(fruit)

# plot fruit tree abundance with x-axis ranked by the average
rank_tp_number_grouped2_mean %>%
  filter(measure == "mean") %>%
  ggplot(aes(x = factor(rank), y = tp_fruit2, color = case_control_group)) +
  geom_jitter(height = 0, width = 0.2, alpha = 0.8) +
  facet_wrap(~Trees_observed) +
  scale_x_discrete(labels = rank_tp_number_grouped2_names) +
  labs(x = "Fruit type",
       y = "Number of trees in bari",
       title = "Number of trees present in a bari grouped by village type") +
  theme_bw() +
...
<<<<<<< Updated upstream
```{r human dropped fruit consumption graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=10}
=======
```{r human dropped winter fruit consumption graph, echo=FALSE, fig.align=, fig.height=5, fig.width=10}
>>>>>>> Stashed changes
# create a new object where average consumption across village types is ranked
...
rank_h_dropped_grouped_mean %>%
  ...
  labs(x = "Fruit type",
       ...
       title = "Proportion of respondants reporting dropped winter fruit\nconsumption by bari members grouped by village type") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.y = element_text(vjust = 1)) +
  scale_color_manual(
...
```{r human winter fruit consumption graph, echo=FALSE, fig.height = 5, fig.width = 10, fig.align = "center"}
# create a new object where average consumption across village types is ranked
...
<<<<<<< Updated upstream
```{r animal dropped fruit consumption graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=10}
=======
```{r animal dropped winter fruit consumption graph, echo=FALSE, fig.align=, fig.height=5, fig.width=10}
>>>>>>> Stashed changes
# create a new object where average consumption across village types is ranked
...
<<<<<<< Updated upstream
```{r bat fruit consumption graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=10}
=======
```{r bat winter fruit consumption graph, echo=FALSE, fig.align=, fig.height=5, fig.width=10}
>>>>>>> Stashed changes
# create a new object where average consumption across village types is ranked
...
=======
```{r bat fruit consumption graph 2, eval=FALSE, include=FALSE}
>>>>>>> Stashed changes
# plot bat fruit consumption with x-axis ranked by the average
#rank_b_fruit_grouped_mean %>%
