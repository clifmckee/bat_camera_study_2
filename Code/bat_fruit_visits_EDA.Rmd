...
fruit_data_vil %>% 
  ...
  # 45 case villages, 30 far control villages, 44 near control villages
```

There are a total of 118 different villages observed. These are split between 45 case, 43 near control, and 30 far control. 
...
fruit_data_vil %>% 
  ...
  dplyr::rename(village_type = case_control_group)

```

The average number of trees observed per village is 1.7 (mix-max = 1-3). Of the 204 trees observed, 80 were located in case villages, 50 were located in far control villages, and 74 were located in hear control villages. 
...
tree_summary_vil %>%
  ggplot(aes(x = reorder(tree_type, -num_trees) , y = num_trees)) +
  geom_bar(stat = "identity") + 
  geom_text(aes(label = num_trees), vjust=-.5) +
  ylim(0, 85) +
  theme_minimal() +
...
fruit_data_vil_sum %>%
  distinct(Tree_number, .keep_all = TRUE) %>% 
    ...
    labs(title = "Number of trees observed by village type and tree species", 
            caption = "Tree types with only 1 or 2 trees observed categorized as other. This includes tamarind, papaya, \nrose apple, cotton tree, indian olive, fig, betel nut, and jack fruit.")
```
...
fruit_data_vil %>%
      group_by(case_control_group) %>%
      summarise(num_tree = n_distinct(Tree_number), 
                num_visits = sum(Number_of_visits)) %>% 
      mutate(percent_visits = round((num_visits / sum(num_visits)), 3), 
             percent_tree = round((num_tree / sum(num_tree)), 3))
...
bat_tree_tot <- fruit_data_vil %>% 
  ...
  summarize(num_visits = sum(Number_of_visits)) %>% 
  mutate(yn_visit = case_when(num_visits > 0 ~ "yes", TRUE ~ "no")) %>% 
  inner_join(fruit_data_vil %>% dplyr::select(Tree_number,Tree_type))

bat_tree_tot <- bat_tree_tot[!duplicated(bat_tree_tot), ]

# Add back village type information
bat_tree_tot <-inner_join(bat_tree_tot, fruit_data_vil %>% 
                            dplyr::select(Tree_number, case_control_group))
...
v_stay <- fruit_data_visits %>% 
  ...
  dplyr::rename(case_control_group1 = case_control_group)

# visits resulting in a contamination
v_cont <- fruit_data_visits %>% 
  group_by(case_control_group) %>% 
  summarise(num_cont = sum(Number_of_contaminations)) %>% 
  dplyr::rename(case_control_group2 = case_control_group)
...
v_stay_tree <- fruit_data_visits %>% 
  ...
  dplyr::rename(Tree_type1 = Tree_type)
...
v_cont_tree <- fruit_data_visits %>% 
  ...
  dplyr::rename(Tree_type2 = Tree_type)
...
avg_cont <- fruit_data_visits %>% 
  ...
  dplyr::rename(case_control_group3 = case_control_group)
