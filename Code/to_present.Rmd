...
```{r Number of contaminations at date palm and fruit trees}
...
```{r average number of date palm trees observed per village}
...
```{r average number of trees observed per village by village type}
...
```{r villages with bat visits to date palm trees}
...
```{r date palms with no bat visits by village type}
...
```{r cumulative and average visit duration by village type 2, echo=TRUE}
...
```{r regroup tree totals with other category, echo=FALSE}
...
```{r Fruit trees with and without bat visits by tree type}
...
```{r fruit tree visits resulting in contamination}
all_data %>% 
  filter(dp_or_fruit == "Fruit", Number_of_visits > 0) %>% 
  summarize(pct_cont = mean(Number_of_contaminations))
```

```{r fruit tree visits resulting in contamination by tree type}
all_data %>% 
  filter(dp_or_fruit == "Fruit", Number_of_visits > 0) %>% 
  mutate(yn_cont = case_when(Number_of_contaminations == 1 ~ "1", TRUE ~ "0")) %>% 
...
```{r average contamination duration boxplot}
...
##### Contamination durations to date palm and fruit trees conbined
...
```{r total villages with community surveys}
