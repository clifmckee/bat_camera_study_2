...
v2 <- vil.total_long %>%
  ...
  mutate(vil_not_observed = total_vil - vil_observed, 
         "perc_vil" = round((vil_observed/total_vil*100), digit = 0))
```
...
all_vil <- all_vil %>% 
  mutate(case_control_group_f = factor(all_vil$case_control_group, levels=c("case", "control, near", "control, far")))
...
all_data <- all_data %>% 
  mutate(case_control_group_f = factor(all_data$case_control_group, levels=c("case", "control, near", "control, far")))
...
all_data %>%
  ...
  facet_wrap(~ case_control_group_f) +
  theme_bw() +
  labs(title = "Bat visit contamination duration (seconds) in buffet villages at date palm trees vs. fruit trees by village type, \nlog transformed", 
...
```{r graph of date palms that received visits by buffet and village type}

dp.labs <- c("Buffet village", "Date palm only village")
names(dp.labs) <- c("Both", "Date palm only")

bat_tree_tot <- bat_tree_tot %>% 
  mutate(case_control_group_f = factor(bat_tree_tot$case_control_group, levels=c("case", "control, near", "control, far")))
...
all_data %>% 
  ...
  group_by(Trees_observed, case_control_group_f, recode_P_NP) %>% 
...
ts.labs <- c("Buffet village", "Fruit tree only village")
names(ts.labs) <- c("Both", "Fruit tree only")
...
tree_summary_vil2 %>% 
    ggplot(aes(x = reorder(tree_type, -num_trees), y = num_trees, fill = case_control_group_f)) +
     ...
     facet_wrap(~Trees_observed, 
                labeller = labeller(Trees_observed = ts.labs)) +
     ylim(0, 40) +
      scale_fill_manual(values = c("case" = "#E69F00",
                                  "control, near" = "#009E73",  
                                 "control, far" = "#56B4E9"
...
bat_tree_tot_f <- bat_tree_tot_f %>% 
  mutate(case_control_group_f = factor(bat_tree_tot_f$case_control_group, levels=c("case", "control, near", "control, far")))
...
```{r date palm cont. duration compared to fruit trees, fig.height=5, fig.width=10}
...
```{r winter fruit tree presence in bari graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=10}
...
```{r human dropped winter fruit consumption graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=6}
...
```{r animal dropped fruit consumption graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=6}
# create a new object where average consumption across village types is ranked
...
```{r bat winter fruit consumption graph, echo=FALSE, fig.align="center", fig.height=5, fig.width=6}
# create a new object where average consumption across village types is ranked
